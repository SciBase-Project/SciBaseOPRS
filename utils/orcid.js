    ...
    fetchUserDetails: function(orcid, callback) {
            ...
            request(get_record_msg, function(error, response, body) {
                if (error && response.statusCode != 200) {
                    ...
                    if (parsedData["orcid-profile"]["orcid-activities"] != null) {
                        ...
                        if (orcid_affiliation != null) {
                            ...
                            for (var i = 0; i < affiliations.length; i++) {
                                var orcid_affiliation_education_obj = {};
                                var orcid_affiliation_employment_obj = {};
                                if (affiliations[i]["type"] == "EDUCATION") {
                                    orcid_affiliation_education_obj["department"] = affiliations[i]["department-name"];
                                    orcid_affiliation_education_obj["role"] = affiliations[i]["role-title"];
                                    orcid_affiliation_education_obj["organization"] = affiliations[i]["organization"]["name"];
                                    education_list.push(orcid_affiliation_education_obj);
                                } else if (affiliations[i]["type"] == "EMPLOYMENT") {
                                    orcid_affiliation_employment_obj["department"] = affiliations[i]["department-name"];
                                    orcid_affiliation_employment_obj["role"] = affiliations[i]["role-title"];
                                    orcid_affiliation_employment_obj["organization"] = affiliations[i]["organization"]["name"];
                                    employment_list.push(orcid_affiliation_employment_obj);
                                }
                            }
                            if (employment_list.length > 0) {
                                current_organization = employment_list[0]["organization"];

                            } else {
                                current_organization = education_list[0]["organization"];
                            }
                        }
                    ...
                    console.log("Orcid education : ", education_list);
                    console.log("Orcid employment : ", employment_list);
                    ...
                    User.findOneAndUpdate({ orcid: orcid }, { bio: bio, researcher_urls: researcher_urls, keywords: keywords, contact_details: contact_details, education: education_list, employment: employment_list, works: works_list, funding: funding_list, fetched_orcid_data: true, current_organization: current_organization }, (err, user) => {
